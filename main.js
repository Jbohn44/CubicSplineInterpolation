...
function System(points) {

	this.equations = [];

	this.initSystem = function () {
		for (let i = 0; i < points.length; i++) {
			let equation = new Equation(points)
			equation.id = i;
	...
	this.cubicSplineCoefficients = function () {
		...
		for (let i = 0; i < points.length - 2; i++) {
			let term1 = (points[i + 2].y - points[i + 1].y) /
				(points[i + 2].x - points[i + 1].x);

			let term2 = (points[i + 1].y - points[i].y) /
				(points[i + 1].x - points[i].x);

			let dValue = 6 * (term1 - term2);
...
function SolveCubicSplineLinearSystem(equations) {
	let solutions = [];
	let solutionSet = [];
	...
	for (let i = solutions.length - 1; i > 0; i--) {
		let solution = solutions[i].r[solutions.length] * solutions[i - 1].r[i] +
	...
	// log solutions system of equations and solutions set
...
function produceDataSetsFromCubicEquations(t, p, dx) {
	let dataSets = [];
	let x = 0;
	let y = 0;
	let value1 = 0;
...
let hCenter = width / 2;  	//horizontal center
let vCenter = height / 2;	//vertical center
let hShift = 100;			// horizontal shift
let vShift = 0;				// vertical shift
let xScale = 16;			// x scaling factor
let yScale = 12;			// y scaling factor
let margin = 10;			// value for empty space border

function graphDataSet(data, points) {
...
// Calling functions...
let s = new System(points);
s.initSystem();
s.cubicSplineCoefficients();

let t = SolveCubicSplineLinearSystem(s.equations);
let dataSet = produceDataSetsFromCubicEquations(t, points, 0.1);
